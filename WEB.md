...
- `RUNS_DIR=/app/runs` для истории запусков в рамках текущего экземпляра.
...
Blueprint настроен на бесплатный `plan: free`, без persistent disk. На free-тарифе история запусков может пропадать после рестарта, redeploy или сна сервиса. Для постоянной истории нужен платный service с disk, смонтированным в `/app/runs`.
